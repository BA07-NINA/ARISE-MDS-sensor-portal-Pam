...
def test_deploy_non_managed_device(api_client_with_credentials):
    ...
    # Create deployment objects that are properly saved to the database
    not_allowed_deployment = DeploymentFactory.build(
        device=device_not_owned, site=site)
    not_allowed_deployment.save()  # Save to get a primary key
    
    # Get serialized data for the deployment
...
def test_deployment_viewer_view_datafiles(api_client_with_credentials):
    ...
    data_file = DataFile(
        ...
        file_format=".jpg",
        file_size=1000,
        recording_dt=dt(1066, 6, 1, 0, 0, 0),  # Within deployment range
        path="test/path",
        local_path="/tmp"
    )
    data_file.save()

    object_url = f'/api/datafile/{data_file.pk}/'

    # User is not a viewer yet, so should get 404
    ...
    # Add user as viewer
    data_file.deployment.viewers.add(user)
